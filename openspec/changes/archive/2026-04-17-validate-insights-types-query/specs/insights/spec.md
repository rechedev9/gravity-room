...
### Requirement: The `GET /insights` endpoint SHALL validate the `types` query parameter against the closed set of known insight types
...
- **Then** the response body includes a stable error identifier for "invalid insight type"
- **And** the response body includes the offending value(s)
- **And** the response body includes the full list of valid values

...
- **API contract**: ElysiaJS exposes `/swagger/json` (non-prod). Web codegen at `apps/frontend/web/codegen/generate-api-types.ts` regenerates `apps/frontend/web/src/lib/api/generated.ts`. There is no CI drift gate, so after changing routes the author must run `pnpm --filter web api:types` with the local API running and commit the result. **Mobile does NOT consume this generated client** — it hand-writes API calls. Unifying is on the roadmap (`packages/api-client`).
...
- **Repository CI workflows**, including `ci.yml`, `validate.yml`, and `security.yml`. Validation is local through Lefthook and explicit pnpm commands; do not reintroduce an aggregate `Validate` check. **Active workflows:** `production-smoke.yml`, `claude.yml`, `claude-code-review.yml`. `production-smoke.yml` checks promoted deep-link status/headers after successful Vercel production deployments and daily.

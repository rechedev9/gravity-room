...
migrations) and `packages/api-client` (endpoint-agnostic JSON transport plus
shared fetch/auth utilities).
...
- **Dependency policy**: `scripts/check-architecture-boundaries.ts` scans web, mobile, and backend runtime source plus the domain, API-client, and database packages. `pnpm run architecture:test` exercises policy fixtures and `pnpm run architecture:check` scans the repository; CI runs both inside the existing `format` job. Web prerender scripts remain outside the web-runtime boundary and may consume database seed fixtures.

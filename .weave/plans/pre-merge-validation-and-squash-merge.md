...
- [x] 6. Add `web` job to `validate.yml`
  **What**: Job that runs on `ubuntu-latest`, checks out, sets up Bun, restores `~/.bun/install/cache` keyed on `bun.lock`, installs at the repo root (`bun install --frozen-lockfile`), then runs the web pipeline.
  ...
  2. `oven-sh/setup-bun@v2` with `bun-version: latest` (or pinned — check `package.json` for any bun version pin; none found so latest is fine).
  3. Cache step: `actions/cache@v4` with `path: ~/.bun/install/cache`, `key: bun-${{ runner.os }}-${{ hashFiles('bun.lock') }}`.
  4. `bun install --frozen-lockfile`
  5. `bun run --filter web typecheck`
  6. `bun run --filter web lint`
  7. `bun run format:check` (root-level, covers all of `.`)
  8. `bun run --filter web test`
  9. `VITE_API_URL=http://localhost:3001 bun run --filter web build` (mirror lefthook env so build does not fail on missing var)
...
- [x] 7. Add `api` job to `validate.yml`
  **What**: Parallel job, same Bun setup + cache. Steps:
  ...
  2. `bun run --filter api typecheck`
  3. `bun run --filter api lint`
  4. `bun run --filter api test`  ← unit tests only; `test:e2e` (DB-backed) deliberately excluded.
...
- [x] 8. Add `analytics` job to `validate.yml`
  **What**: Parallel job using Python:
  1. `actions/checkout@v4`
  2. `actions/setup-python@v5` with `python-version: '3.12'` and `cache: 'pip'`, `cache-dependency-path: apps/analytics/requirements*.txt`
  3. `pip install -r apps/analytics/requirements.txt -r apps/analytics/requirements-dev.txt` (run in `apps/analytics` working-dir)
  4. `ruff check .` (working-dir `apps/analytics`)
  5. `ruff format --check .` (optional; only if step 4 baseline passes — otherwise skip in v1 and add later)
  6. `pytest -q` (working-dir `apps/analytics`)

...
| `apps/backend/api/src/analytics/`            | backend  | TS insight pipelines + Cron compute (e1RM, frequency, summary, volume, forecast, plateau, recommendation) | TS (stats, ISO-week, IRLS logistic)           | `bun run test:api` (incl. golden-file parity)                              |
| `apps/backend/api/src/analytics/pipelines/`  | backend  | Per-insight pipeline functions ported from the former Python service                                      | TS                                            | `bun test apps/backend/api/src/analytics`                                  |
...
| "Where is the analytics insights logic?"           | `apps/backend/api/src/analytics/pipelines/` (TS; computed via the `/api/internal/analytics/compute` cron route) |

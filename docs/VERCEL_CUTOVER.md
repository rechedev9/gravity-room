...
The migrated architecture is one Vercel project where the Vite/React PWA ships as static output and the Elysia API runs as a Node serverless function at the root catch-all `api/[...path].ts`.
Read [`../MIGRATION_PLAN.md`](../MIGRATION_PLAN.md) for the decision record and [`../.env.example`](../.env.example) for the full env template.
Every step below is a manual, out-of-repo action unless it says otherwise.

## Required environment variables

The table below is the canonical list pulled from the API code and `.env.example`, with the value or source for each Vercel environment.
...
- `DIRECT_DATABASE_URL` is the Neon DIRECT (non-pooled) connection string for the same branch, used only by the build-time `db:deploy` migration step.
...
Leave `CORS_ORIGIN` empty in both environments and set `TRUSTED_PROXY=true` in both.
...
Save the change and allow a few minutes for it to propagate, because Google Sign-In on the web stays broken until the new origin is authorized.
...
The CI workflow `.github/workflows/ci.yml` now exposes the job names `Web`, `API`, `Domain`, `Mobile`, `Format`, and `OpenAPI client drift`.
In the GitHub repository settings open Branches, edit the protection rule for `main`, and set the required status checks to exactly those six job names.
Remove any stale required checks left over from the deleted Railway and per-service validate workflows so a merge is not blocked on checks that no longer run.

 ...
 * Mirrors the prototype `.k` voice: 10px JetBrains Mono, tight intentional tracking.
 */
export function Kicker({ children, index, noRule = false, className }: KickerProps): ReactNode {
  return (
    <div
      className={cn(
        'flex items-center gap-2.5 font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-label',
        className
      )}
    >

...
SELECT setval('workout_results_id_seq', GREATEST(COALESCE((SELECT MAX(id) FROM workout_results), 0), 1), COALESCE((SELECT MAX(id) FROM workout_results), 0) > 0);
...
SELECT setval('undo_entries_id_seq', GREATEST(COALESCE((SELECT MAX(id) FROM undo_entries), 0), 1), COALESCE((SELECT MAX(id) FROM undo_entries), 0) > 0);

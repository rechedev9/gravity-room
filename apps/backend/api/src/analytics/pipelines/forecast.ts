...
export function forecastSlot(records: readonly WorkoutRecord[]): E1rmForecastPayload | null {
  ...
  // Guard against NaN or Inf in the regression results, mirroring the
  // `np.isfinite(slope) and np.isfinite(intercept)` guard in V2's
  // ml/forecast.py: a non-finite fit carries no meaningful forecast.
